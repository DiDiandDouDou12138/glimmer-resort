...
**© 2026 DiDiandDouDou12138　保留所有权利。**

本仓库公开源码，但**不是开源项目**——未采用任何开源许可证，即默认保留所有权利。
公开可见不等于授予使用许可。完整条款见 **[COPYRIGHT](COPYRIGHT)**，摘要如下：

| | |
|---|---|
| ✅ 可以 | 游玩、阅读源码用于个人学习、克隆到本地运行；评介本作时引用少量代码或截图（请注明作品名与原始地址） |
| ❌ 不可以 | 转载、镜像、重新部署到其他域名、二次分发、改编或制作衍生作品、任何商业用途、移除版权与免责声明、单独抽取美术／音频／剧情文本／谜题设计用于其他项目 |

需要授权请在本仓库提交 Issue 说明用途。

本作部分图片与配音由 AI 生成工具制作，经人工筛选与编辑后接入；这些素材是本作的组成部分，同样受上述条款约束。

**防伪**：唯一官方地址 <https://glimmer.mimoris.cn>，唯一官方源码仓库即本仓库。
其他域名上的副本均非官方发布——页面内置域名校验，在非官方地址打开时页脚会显示告警。
